...
Write-Host "Pushing source to main..." -ForegroundColor Cyan
if (-not $repoExists) {
...
$prevEap = $ErrorActionPreference
$ErrorActionPreference = "SilentlyContinue"
& $gh api repos/ShekharDan/prep-duel/pages -X POST -f "build_type=legacy" -f "source[branch]=gh-pages" -f "source[path]=/" *> $null
if ($LASTEXITCODE -ne 0) {
  & $gh api repos/ShekharDan/prep-duel/pages -X PUT -f "build_type=legacy" -f "source[branch]=gh-pages" -f "source[path]=/" *> $null
}
$ErrorActionPreference = $prevEap
